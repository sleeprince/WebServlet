	  ...
	  <h1 class="display-1 text-center">사용자 수정</h1>
		...
		<input type="hidden" name="number" value=<%=user.getNo()%>>
		  <div class="mb-3 mt-3">
		    ...
		    <input type="text" class="form-control" id="name" placeholder="<%=user.getName()%>" name="name">
		  </div>
		  <div class="mb-3 mt-3">
		    <label for="email" class="form-label">이메일:</label>
		    <input type="email" class="form-control" id="email" placeholder="<%=user.getEmail()%>" name="email">
		  </div>
		  <div class="mb-3">
		...
		<div class="d-flex">
		  <div class="p-2 flex-fill d-grid">
			...
			<button type="submit" class="btn btn-primary" onclick="javascript: frm.action = 'UpdateData'">저장</button>
		  </div>
		  <div class="p-2 flex-fill d-grid">
			<!-- <a href="Select" class="btn btn-primary">취소</a> -->
			<button type="submit" class="btn btn-primary" onclick="javascript: frm.action = 'Select'">취소</button>
		  </div>
		</div>
		</form>
	</div>
